  ...
  <div class="w-full lg:w-1/2 py-4">
    ...
    {#if $sortedEvents?.length === 0}
      <div class="px-3 flex flex-col space-y-8">
        ...
        <TouchableOpacity 
          on:click={() => openEditor()}
          classNames="py-4 px-3 rounded-lg font-semibold select-none text-sm uppercase bg-opacity-10 dark:bg-opacity-20 text-indigo-500 bg-indigo-500 dark:bg-lime-400 dark:text-lime-400">Create event</TouchableOpacity>
      </div>
    {/if}
  </div>
</div>
